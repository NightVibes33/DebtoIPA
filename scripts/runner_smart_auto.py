#!/usr/bin/env python3
"""Build only IPAs that contain an original app executable from the input DEB.
...
def load_engine() -> dict[str, Any]:
    ns: dict[str, Any] = {"__name__": "debtoipa_runner_engine", "__file__": str(PUBLIC / "converter.py")}
    ...
    run(PUBLIC / "port_mode.py", gzip.decompress(base64.b64decode(encoded)).decode("utf-8"))
    return ns
...
def validate_ipa_bytes(data: bytes) -> dict[str, Any]:
    with zipfile.ZipFile(io.BytesIO(data)) as archive:
        ...
        if len(plists) != 1:
            raise RuntimeError(f"IPA must contain exactly one top-level app; found {len(plists)}.")
        plist_name = plists[0]
...
def extract_ipas(result_zip: Path, output_dir: Path) -> list[dict[str, Any]]:
    ...
    with zipfile.ZipFile(result_zip) as archive:
        for name in archive.namelist():
            ...
            results.append({"name": destination.name, "size": len(data), "validation": validate_ipa_bytes(data)})
    return results
...
def extract_deb_payload(deb: Path, destination: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="debtoipa-ar-") as temporary:
        ...
        subprocess.run(["tar", "-xf", str(payload), "-C", str(destination)], check=True)
...
def find_original_apps(root: Path) -> list[Path]:
    ...
    for app in root.rglob("*.app"):
        ...
        try:
            ...
            if not executable or not binary.is_file() or binary.read_bytes()[:4] not in MACHO:
                continue
        except Exception:
            continue
        relative = app.relative_to(root).as_posix().lower()
        score = (100 if "/applications/" in f"/{relative}" else 0) - len(app.relative_to(root).parts)
        found.append((score, app))
    return [app for _, app in sorted(found, key=lambda value: (-value[0], str(value[1])))]
...
def package_original_app(app: Path, output_dir: Path, source_name: str, options: dict[str, Any]) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="debtoipa-original-") as temporary:
        ...
        plist_path.write_bytes(plistlib.dumps(plist, fmt=plistlib.FMT_BINARY, sort_keys=False))
...
def append_ipas(result_zip: Path, output_dir: Path, ipas: list[dict[str, Any]]) -> None:
    with zipfile.ZipFile(result_zip, "a", zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
    ...
    try:
        ...
        if summary["ipas"]:
            ...
            with tempfile.TemporaryDirectory(prefix="debtoipa-payload-") as temporary:
                ...
                if not apps:
                    summary.update(verdict="blocked-no-standalone-app", stage="report-only", originalBinaryPackaged=False, featureComplete=False)
                    exit_code = 2
                else:
                    summary["ipas"] = [package_original_app(app, output, args.source_name, options) for app in apps]
        ...
        (output / "runner-summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
        (output / "README.txt").write_text(f"DebToIPA runner failed\n\n{error}\n", encoding="utf-8")
